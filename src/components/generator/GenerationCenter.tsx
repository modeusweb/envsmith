...
interface GenerationCenterProps {
  ...
  mode: ExampleMode;
  /** Closing plays the exit animation, so keep this popup rendered and flip this flag instead. */
  open: boolean;
  onModeChange: (m: ExampleMode) => void;
  onClose: () => void;
}

export function GenerationCenter({ variables, mode, open, onModeChange, onClose }: GenerationCenterProps) {
  const [target, setTarget] = useState<SchemaTarget>("zod");

  // Nothing is generated while the popup is closed (even though it stays mounted).
  const example = useMemo(
    () => (open ? generateExample(variables, { mode, includeDescriptions: true }) : ""),
    [open, variables, mode]
  );
  const schema = useMemo(
    () => (open ? generateZodSchema(variables, { target }) : ""),
    [open, variables, target]
  );
  const docs = useMemo(() => (open ? generateDocumentation(variables) : ""), [open, variables]);
  ...
  return (
    <Modal
      ...
      variant="panel"
      className="max-w-6xl rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-xl"
    >
      <div className="flex items-center justify-between gap-2 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-4 py-3 shrink-0">
