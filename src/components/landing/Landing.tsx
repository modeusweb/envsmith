  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 flex-1 w-full">
      <section className="text-center pt-14 sm:pt-20 pb-10">
        ...
        <p className="mt-4 text-lg sm:text-xl font-medium tracking-tight text-balance">
          Keep your .env and .env.example in sync.
        </p>
        <p className="mt-4 text-base text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto text-balance">
          Generate a safe <code className="font-mono text-[0.92em]">.env.example</code> and a validation
          schema from your local .env — in your browser, with secrets masked and nothing uploaded.
        </p>
      </section>
      ...
      >
        ...
        <div className="mt-4 grid gap-6 sm:grid-cols-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
          <div>
            <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Parses dotenv files</h3>
            <p className="mt-1.5">
              Quoted values, <code className="font-mono text-xs">export</code> prefixes, comments and values
              containing equals signs — with line-numbered, plain-language parse errors.
            </p>
          </div>
          <div>
            ...
            <p className="mt-1.5">
              Each variable gets an inferred type and a secret classification. Secrets stay masked and are
              never written into generated files.
            </p>
          </div>
          <div>
            <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Generates safe output</h3>
            <p className="mt-1.5">
              A clean .env.example plus Zod, TypeScript or JSON Schema validation you can copy or download.
            </p>
          </div>
        </div>
        <div className="mt-6 text-sm">
          <Link
            href="/help"
            className="inline-flex items-center gap-1.5 font-medium text-sky-600 dark:text-sky-400 hover:underline focus-visible:outline-2 focus-visible:outline-sky-500"
          >
            How it works
            <ArrowRightIcon className="size-4" aria-hidden />
          </Link>
